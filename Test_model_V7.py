...
from tensorflow.keras.layers import Flatten, Conv2D, MaxPool2D, BatchNormalization, LayerNormalization
...
#%%Get Model
def get_model(model_nr = 1, norm_layer = "", kernel_size=(3,3), pool_size=(4,4), first_filters=32, second_filters=64):
    
    if model_nr != 1 and norm_layer == "":
        print("Please define a normalization layer or choose model_nr=1 (no normalization).")
        return
...
results = {}
for i in range(5):
    history = model_training(model_weight, train_gen, val_gen, 'Model_1.' + str(i+1), verbose=1)
    auc_score = ROC_analysis(model_weight, val_gen)
